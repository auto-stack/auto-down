...
| 4 | 表格列宽拖拽 | ext 桥 useTableColumnResize（DOM 测量） | 固定布局（无测量通道） | **PLAN-045**（execution_done 待 review） | DEBTS.md 040 表格列宽行（🟡） |
| 5 | 主题观感 | 浅色：toolbar #fff / 文字 #111827（app.at style 块） | 深色默认主题（vm-two-columns.png 实证） | **W2 预留**，前置 auto-lang **PLAN-527** T8（dark/theme 机制） | app.at style 块 .toolbar/.app 颜色段 vs vm-two-columns.png |
| 6 | 初始文档种子 | ext `initial_content()` 载 src/content.ts 全文档 | ext 桩返回 ""（空文档起步，打字见联动） | **W3 预留**，前置 auto-lang ext 资产机制或 DSL 多行字面量立项 | app.at 头注 content 段（DSL 无多行模板字面量）；DEBTS.md 040 ext 桩行 |
...
| 8 | CustomScrollbar 观感/拖拽 | 自绘 thumb + 拖拽 | 三测量数据已接（043）；拖拽发射面实证（T2 smoke drag 组过）；thumb 观感差异残留 | 数据面 ✅ **PLAN-043 收口**；thumb 观感段并入 **W2**（主题族） | DEBTS.md 040 CustomScrollbar 销号行；vm-smoke.mjs 拖拽断言；vm-drag-before/-after.png |
...
| 11 | ext 桩告警五符号 | initial_content/is_vue/logSave/logCancel/useDemoAppBridge 真实现 | no-op 平台桩 + 运行时各告警一次（**预期行为**） | **显式豁免维持**（`AUTO_VM_EXT_STUBS=0` 可复原硬错误） | DEBTS.md 040 ext 桩行；README「Ext stub warnings」段 |
...
- `autodown`：class 整串不消费 → 渲染面板 `py-4 px-5` 内边距 VM 缺席，
  并入差异 #5（观感族，W2/PARITY-527 补齐），不阻塞结构验收。
...
| 波次 | 内容 | 前置 | 吞吐差异项 |
|------|------|------|-----------|
| W2 | 主题对齐（VM 浅色两栏） | auto-lang PLAN-527 T8（dark/theme） | #5、#8 观感段、#4 渲染面板内边距（py-4 px-5 VM 消费臂） |
| W3 | 初始文档种子 | auto-lang ext 资产机制 或 DSL 多行字面量立项 | #6 |
