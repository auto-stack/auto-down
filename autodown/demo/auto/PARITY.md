...
| 2 | 滚动同步 | useSyncedScroll（DOM 块级映射 + spacer 顶对齐，PLAN-063 零改动） | PLAN-063：cmd/view 状态分离（写臂绑 \*_top_cmd，echo 写 \*_top_view）+ **单向左→右块锚定命令式同步**（v2 rust 直写唯一同步源——锚槽 content_y → right_top_cmd 直写；v1 比例臂已随 T-04d-2 摘除）；043 双向比例联动退役（单变量双臂闭环实测帧率自激，063 §4.2） | ✅ **PLAN-063 T1/T3**（vm-smoke 组 4 新契约：反振荡计数冻结 + 单向性 + 拖拽不回退 + 幂等复滚全绿）；✅ **引擎侧两缺陷已修复复归**（auto-lang auto-down-dev abd6aeca8 _D 族标签驱动弹栈 + 1a828a2cf 空体引号 handler emit 桥/剥前导点；实机 vm-smoke 全量臂绿：offset_y=240.9 跟随 + 拖拽 968.7）；✅ **T-04d-2 收口（2026-09-13）**（auto-lang 334d44e53 树委托根因修复 + 6d3cd294e VNode 透传 + d85056a AC-06 四腿断言；vm-smoke 21/21 两连净窗，滚左 700→锚块 9→右栏 echo 跟随 846.79，顶对齐截图在案） | demo/auto/src/front/app.at model/handler 注；vm-smoke.mjs 组 4；DEBTS.md 063 行；docs/plans/063 §4.2 + 阶段记录 3 |
...
| 8 | CustomScrollbar 观感/拖拽 | 自绘 thumb + 拖拽 | 三测量数据已接（043）；拖拽发射面实证（T2 smoke drag 组过）；thumb 观感差异残留 | 数据面 ✅ **PLAN-043 收口**；✅ **PLAN-058 T12 收口**（thumb 观感：bg-black/30 命名色+透明度 VM 不解析→改 bg-[#000000]/30 arbitrary 形态（plan 503 支持），w-[8px]/rounded 原生可解析；实机 thumb 灰色可见）；✅ **PLAN-063 期回归已修复**：Move 守卫 float 比较恒假系 auto-lang _D 族解码缺陷（abd6aeca8 修复），拖拽全量断言复归（实机 968.7 跳变）；smoke 组 4(d) 降级臂保留为引擎健康信号门控 | DEBTS.md 040 CustomScrollbar 销号行 + 063 引擎转介行；vm-smoke.mjs 组 4(d)；vm-drag-before/-after.png |
...
| 12 | 编辑面能力差 | @autodown/engine WYSIWYG 块家族（气泡/斜杠菜单/undo/节点视图） | cosmic-text 块编辑壳（块粒度编辑；048 补齐跨块选区/跨容器合并/行首输入规则/undo 钉死——编辑行为面收口）；✅ **斜杠菜单分项收口（PLAN-069）**：原生 `/` 弹层候选 23 项（触发七门/键位路由/子串过滤/点选/骨架插入；结构操作不入 undo=048 口径），契约冻结 demo/auto/README.md「VM 轨 slash 菜单契约」节，实机 vm-smoke [slash] 组 + vm-069-probe 五臂（vm-069-slash-menu.png），差额 5 项（Image/Query/Mermaid/Math/Block link）与五差异注记入 DEBTS 069 行 | **长期线**，清册转介——台账「块组件契约/WYSIWYG」目标族主战场，非波次化对象；048 T1 产出的 vue↔rust 编辑语义用例对照表（26 组三态映射，计划复审记录在册）为 #12 长期线的底册输入；斜杠菜单分项已收口（069），气泡菜单/节点视图/行内规则（#20）仍留长期线 | packages/engine/EDITOR-CONTRACT.md；specs 台账 goals 族；PLAN-048 复审记录对照表；PLAN-069（README slash 契约节 + DEBTS 069 行） |
...
| 19 | 块首点光标落点（文本叶块点击交接 / 容器点击归属 / 表格单元格聚焦） | ✅ **本次（2026-09-09）网页轨落地**：click-caret 单槽通道（点 / 明文偏移 / cellId 三类载荷）+ `resolveClickHit`（叶包装 ↔ 模型原子子叶按文档序配对、表格行列定位）+ `selectBlock` 深选覆盖 + RichTextHost / 表格编辑面挂载消费；实测修复前 H2 点 `i`(offset 4)→caret 11、容器点第 2 项→焦点落第 1 项、表格编辑面不聚焦，修复后逐项 = 点击位 | ✅ **PLAN-061 T-04/T-05 收口（2026-09-09）**：单趟字形命中实证达标——vm-061-probe.mjs ALL PASS（A1 标题中线 / A2 段落 x=40→off5·x=120→off16 字形单调含行内标记 / A3+A4 列表第 2 项独立点击带 / A5 引用中线 / A6 Callout 正文 / A7 Details 正文·编辑臂恒展开 / B1 表格 Barbaz cell 几何命中 off2→8 / B2 fence 代码行字形级 / B3 行尾 Down 无幻影尾行）+ core.rs 三单测钉死（mouse_click_leaf_lands_caret_at_clicked_glyph / mouse_click_list_lands_on_clicked_item / mouse_click_table_lands_on_clicked_cell）；观察两项处置（2026-09-09 用户裁定）：①空白点击最近块回落 → **对齐网页轨**（VM 侧小计划进行中，hit_test 撤最近块回落）；②Details 编辑臂恒展开 → **显式豁免登记（永久差异）**——编辑臂正文恒可见可点属编辑器 UX 裁定（全展开编辑范式），只读臂保持折叠语义 | ✅ **PLAN-061 收口**（双轨语义对齐裁定：VM 天然达标零逻辑改动，T-05 走单测钉死路径；docs/plans/061-click-caret-parity.md） | engine src/editor/engine/click-caret.ts（:64/76/101/168/181/212/224）；src/editor/components/EngineEditor.vue:617/828；demo/e2e/block-click-caret.spec.ts（8）+ codeblock-click-caret.spec.ts（2）；vm-061-probe.mjs + vm-061-para-caret/-table-cell/-fence-caret.png；auto-lang autodown_editor/core.rs（handle_mouse_press:1162 单趟命中 + T-05 三测） |

| 20 | 行内字体编辑（markdown input rules + bubble 撤销通道 + B/I/U 快捷键） | ✅ **PLAN-062 网页轨落地（2026-09-09）**：键入 `**agc**`/`*x*`/`~~x~~`/`` `x` `` 闭合即 span 变换、光标落 mark 外（AC-01/02）；一次 undo 回到原 marker 文本（AC-03）；Code 区不触发、转义 opener/closer 不触发、`**a*` 不被 `*` 截胡（AC-04）；caret 入已格式化 token → bubble 弹出 + active 高亮 + 点按解除（AC-05）；Ctrl/Cmd+B/I/U 选区 toggle（AC-06 选区半）；**无选区 stored-mark 的「进入」方向未实现**（退出/解除方向已通，bubble 可见化在册）——登记为后续增量 | ⏳ rust CodeEditorCore 独立输入栈，行内规则未实现（原 #12 登记面延续，双轨不静默漂移） | ⏳ **PLAN-062**（网页轨已落地；VM 轨是否对齐另立裁定） | engine src/editor/engine/inline-input-rules.ts（matchInlineRule 纯匹配器 + fireInlineRuleOn 单 undo 变换）；src/parser/block-model.ts spansInsert mark 末尾边界=外插（AC-02 模型基础）；host-controller onInput 接线；rich_text_host_ext caretToOffset ZWSP 锚 + hostInput caret 精确 resync；bubble_menu_ext activeMarksAtCaret + derive 折叠 caret 通道；e2e inline-input-rules.spec（5）+ 单测 inline-input-rules.test（12） |
