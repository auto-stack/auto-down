...
| 2 | 滚动同步 | useSyncedScroll（DOM 块级映射 + spacer 顶对齐，PLAN-063 零改动） | PLAN-063：cmd/view 状态分离（写臂绑 \*_top_cmd，echo 写 \*_top_view）+ **单向左→右命令式同步**（v1 比例臂，v2 块锚定走 rust 直写转介）；043 双向比例联动退役（单变量双臂闭环实测帧率自激，063 §4.2） | ✅ **PLAN-063 T1/T3**（vm-smoke 组 4 新契约：反振荡计数冻结 + 单向性 + 拖拽不回退 + 幂等复滚全绿）；🟡 同步臂与拖拽受 **T-02 引擎缺陷门控**（DEBTS 063 行：handler 内 f64 算术/比较坍缩），修复后断言自动复归 | demo/auto/src/front/app.at model/handler 注；vm-smoke.mjs 组 4；DEBTS.md 063 行；docs/plans/063 §4.2 |
...
| 8 | CustomScrollbar 观感/拖拽 | 自绘 thumb + 拖拽 | 三测量数据已接（043）；拖拽发射面实证（T2 smoke drag 组过）；thumb 观感差异残留 | 数据面 ✅ **PLAN-043 收口**；✅ **PLAN-058 T12 收口**（thumb 观感：bg-black/30 命名色+透明度 VM 不解析→改 bg-[#000000]/30 arbitrary 形态（plan 503 支持），w-[8px]/rounded 原生可解析；实机 thumb 灰色可见）；🟡 **PLAN-063 期引擎回归在册**：Move 守卫 `.scrollHeight > .clientHeight` 为 handler 内 float 比较，T-02 缺陷（DEBTS 063 行）下恒假 → 拖拽不动（**旧 app.at 同证**，非 063 引入）；smoke 组 4(d) 降级臂在册，引擎修复后复归全量断言 | DEBTS.md 040 CustomScrollbar 销号行 + 063 引擎转介行；vm-smoke.mjs 组 4(d)；vm-drag-before/-after.png |
