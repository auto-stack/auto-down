...
| 2 | 滚动同步 | useSyncedScroll（DOM 块级映射 + spacer 顶对齐，PLAN-063 零改动） | PLAN-063：cmd/view 状态分离（写臂绑 \*_top_cmd，echo 写 \*_top_view）+ **单向左→右命令式同步**（v1 比例臂，v2 块锚定走 rust 直写转介）；043 双向比例联动退役（单变量双臂闭环实测帧率自激，063 §4.2） | ✅ **PLAN-063 T1/T3**（vm-smoke 组 4 新契约：反振荡计数冻结 + 单向性 + 拖拽不回退 + 幂等复滚全绿）；✅ **引擎侧两缺陷已修复复归**（auto-lang auto-down-dev abd6aeca8 _D 族标签驱动弹栈 + 1a828a2cf 空体引号 handler emit 桥/剥前导点；实机 vm-smoke 全量臂绿：offset_y=240.9 跟随 + 拖拽 968.7） | demo/auto/src/front/app.at model/handler 注；vm-smoke.mjs 组 4；DEBTS.md 063 行；docs/plans/063 §4.2 |
