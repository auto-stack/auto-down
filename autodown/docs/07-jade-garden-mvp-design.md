...
参考用户提供的 Obsidian 与 LogSeq 截图，对 MVP 界面做以下具体约定。

### Obsidian 截图（深色主题）

- **最左侧 Ribbon**：一列垂直图标（新建笔记、文件树、搜索、书签等），点击切换左侧边栏内容。
- **左侧边栏**：上半是视图切换后的内容区（如文件树），下半可固定“最近文件 / 标签”。
- **文件树**：
  - 文件夹可展开/折叠。
  - 当前打开文件高亮。
  - 支持嵌套目录（如 `webgpu/mdn/examples/pics/README`）。
  - 叶子节点若是 Canvas 会标注 `CANVAS` 标签。
- **顶栏**：
  - 左侧：文件树展开/折叠按钮、当前页 Tab（如 `README`）。
  - 中间：面包屑 `fcg / README`、前进/后退按钮。
  - 右侧：布局切换、更多菜单、窗口控制。
- **主区域**：当前是渲染/阅读视图，显示标题、正文、目录、WikiLink 等。
- **底栏**：
  - 左下角：当前 vault 名 `graphics`、帮助、设置图标。
  - 右下角：反向链接数（`2 条反向链接`）、词数、字符数。

### LogSeq 截图（浅色主题）

- **左侧导航**：图标 + 文字的大按钮列表（日志、白板、记忆卡片、图谱视角、全部页面、收藏、最近使用），底部 `+ 新建`。
- **主区域**：日期页作为默认首页；块级大纲编辑；`[[...]]` WikiLink 可直接点击跳转；支持 `((block-id))` 块引用（MVP 可暂不实现）。
- **提示条**：顶部有黄色提示横幅，用于非阻塞通知（如“当前是示例图谱，修改不会被保存”）。

### 对 Jade Garden MVP 的启示

1. **左侧采用 Ribbon + 内容面板两层**：最左一列图标切换面板，比 Obsidian 更紧凑。
2. **默认深色主题**，跟随 Obsidian 的沉浸式阅读体验；同时提供浅色主题切换。
3. **保留底栏状态栏**：显示反向链接数、词数、字符数、当前工作区名。
4. **顶部面包屑 + Tab 页签**：让用户清楚当前位置，支持多文档并行。
5. **非阻塞通知条**：用于提示保存状态、索引更新、导入进度等。
...
├────┴──────┬───────────────────────────────────────────────────────┬────────────────┤
│           │  README  │  Topic  │  +                                │                │
│  Ribbon   │  fcg / README                      ← →  🌐  ⋮         │   Right        │
│           ├───────────────────────────────────────────────────────┤   Sidebar      │
│  Left     │                                                       │   (collapsible)│
│  Sidebar  │                   主编辑区 / 预览区                    │                │
│  (Files/  │                  (双栏：左编右看)                       │   • Backlinks  │
│   Search/ │                                                       │   • Outlinks   │
│   Recent) │                                                       │   • Outline    │
│           │                                                       │   • Properties │
│           │                                                       │                │
├───────────┴───────────────────────────────────────────────────────┴────────────────┤
│  graphics          ?  ⚙️                      2 条反向链接   136 词   303 字符      │  <- 底栏
└─────────────────────────────────────────────────────────────────────────────────────┘
```
...
- 右侧：
  ...
  - 图视图切换按钮。
  - 深色/浅色切换按钮。
  - 更多菜单（后续扩展：设置、导入、重新索引）。

### 最左侧 Ribbon

- 一列垂直图标按钮，固定宽度（约 44px）。
- 点击后切换左侧边栏内容：
  - 文件树
  - 搜索
  - 最近文件
...
- **页签栏（Tabs）**：显示已打开的文件，支持关闭、切换；未保存的文件名带圆点标记。
- **面包屑（Breadcrumb）**：页签下方显示当前文档路径，如 `fcg / README`，支持点击父级跳转。
- **编辑/预览双栏**：
