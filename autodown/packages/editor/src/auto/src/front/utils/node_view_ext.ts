...
// ../math_inline_node_view.at). Imported via
// `use { component/fn: ... }`; the Auto build copies it into the gen project
// as src/ext/src/front/utils/node_view_ext.ts, and the generated SFCs
// (copied to src/node-views/) import it from
// ../auto/src/front/utils/node_view_ext.
//
// What lives here genuinely cannot be expressed in the widget DSL:
//
// 1. NodeViewWrapper / NodeViewContent — tiptap's node-view components,
//    re-exported from src/composables/tiptapNodeView.ts via a relative path
//    that resolves both in the editor package (real @tiptap/vue-3
//    re-export) and in the gen project (a stub mirrored in by the regen
//    script — the gen project does not depend on @tiptap/*). Same trick as
//    the BubbleMenu wrapper (see src/auto/README.md).
...
//    are carried as data and rendered via `dyn (...)` (same trick as the
//    SlashMenu/BubbleMenu icons). Rendered DOM identical to the originals.
...
export { NodeViewWrapper, NodeViewContent } from '../../../../composables/tiptapNodeView'
export {
  renderKatexPreview,
