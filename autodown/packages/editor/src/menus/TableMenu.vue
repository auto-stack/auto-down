<template>
  ...
  >
    <div class="autodown-table-menu-group">
      <button class="autodown-table-menu-btn" :title="titles?.addRowBefore ?? 'Add row above'" @click="run('addRowBefore')">
        <ArrowUpToLine :size="13" />
      </button>
      <button class="autodown-table-menu-btn" :title="titles?.addRowAfter ?? 'Add row below'" @click="run('addRowAfter')">
        <ArrowDownToLine :size="13" />
      </button>
      <button class="autodown-table-menu-btn" :title="titles?.addColumnBefore ?? 'Add column left'" @click="run('addColumnBefore')">
        <ArrowLeftToLine :size="13" />
      </button>
      <button class="autodown-table-menu-btn" :title="titles?.addColumnAfter ?? 'Add column right'" @click="run('addColumnAfter')">
        <ArrowRightToLine :size="13" />
      </button>
    </div>
    <div class="autodown-table-menu-divider" />
    <div class="autodown-table-menu-group">
      <button class="autodown-table-menu-btn" :title="titles?.deleteRow ?? 'Delete row'" @click="run('deleteRow')">
        <Trash2 :size="13" />
      </button>
      <button class="autodown-table-menu-btn" :title="titles?.deleteColumn ?? 'Delete column'" @click="run('deleteColumn')">
        <Eraser :size="13" />
      </button>
      <button class="autodown-table-menu-btn danger" :title="titles?.deleteTable ?? 'Delete table'" @click="run('deleteTable')">
        <X :size="13" />
      </button>
    </div>
  </div>
...
const props = defineProps<{
  editor: Editor
  titles?: Partial<Record<'addRowBefore' | 'addRowAfter' | 'addColumnBefore' | 'addColumnAfter' | 'deleteRow' | 'deleteColumn' | 'deleteTable', string>>
}>()
