...
| `.code-block-container.autodown-block-placeholder.is-loading` + `pre[aria-busy="true"]` | 开放 fence 骨架（含 ```` ```mermaid ```` 开 fence；等高占位 min-height 5.5rem；闭合翻转即摘除，plan 032）；**plan 033 起该面板为 CodeBlockWidget 的 view/stream 模式**（CodeBlockMenu 点击契约不变）。**plan 039 T7 起编辑态根类并容器全链**（`code-block-container … autodown-codeblock-node`——不再是独立宿主链；外部 badge 角标删除，语言入口为标题栏内触发器 `.code-header-trigger`，见下行；编辑面 `.autodown-code-editor`（+ `.code-editor-highlight` overlay pre / `.code-editor-textarea`）不变） | demo stream-tri-state e2e |
| `.code-block-header` > `.code-header-trigger[data-codeblock-language-badge]`（+ `.code-header-title`/`.code-header-caret`） | Fence 标题栏与语言切换触发器（**plan 039 T7/T13**）：双模式（edit/view/stream）同构由 CodeBlockWidget 模板直出——T14 退役的头部注入器（addCodeBlockHeaders/COPY_ICON）由此链取代；预览态 caret hover 显现（`opacity:0` 基态），点击弹 CodeBlockMenu 语言下拉、选中写回标题 | demo code-block-parity e2e（"language trigger lives in the title bar" 用例） |
| `[data-codeblock-copy-btn]` / `[data-codeblock-expand-btn]`（`.code-action-btn`） | Fence 复制/折叠动作按钮（**plan 039 T14 回归**：家族 widget 吸收 builtin 面板时 actions 丢失的修复面；CodeBlockMenu 事件委托 `closest` 消费，注入器退役后为唯一点击通道） | demo code-block-parity e2e + inline-marks/undo/host-protocol/screenshot |
