// Parity reference for src/streaming.generated.ts (plan 008, Phase 1).
//
// Verbatim copy of the hand-written useStreamingDocument.ts as of the
// migration commit. The ONLY difference is the added `export` on
// buildSegments so the parity test can drive it directly. Do not "fix" or
// modernize anything here — when the generated implementation is proven
// equivalent and this reference has served its purpose, it can be deleted.
//
// The stickyPropsCache module state is intentional: both implementations keep
// their own cache, and the parity test feeds both the same input sequence so
// the caches must evolve identically.
