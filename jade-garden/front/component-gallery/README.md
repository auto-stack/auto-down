...
| D-8 | map 括号写 VM 轨静默吞 handler（078 P-11 转录） | **编译器缺陷，handoff auto-lang 修复**：`.m[k]=v` 读臂有 auto.hashmap.get 路由、写臂无条件发 SET_ELEM（数组专用）→ map 落 RuntimeError "Invalid array ID" 被 VM-HANDLER 吞、写后语句全不执行（字面量/var 键同病）；点号写/括号读/msg 链/全量字面量重赋四臂绿。修复前 VM 轨禁括号写（twin 用点号写/重赋 + 播种投影）；最小复现+实录=attachments/078-vm-map-write-repro/ | 074-sink-mode.md §7.0 P-11；DEBTS 078 行 |
| D-9 | slider/checkbox 词位 VM 轨形态（078 P-12/P-13 转录） | slider=**VM aura 轨缺臂静默丢弃** + vue 轨无 type="range"（a2r 轨 View::slider 完整——补全面非新造，随 canvas 场景契约独立计划）；checkbox=双轨可用（VM toggle 动作→on_toggle msg，vue 自带 type+checked）——twin 布尔投影以 checkbox 通道驱动，slider 交互不进断言（Q-4 收敛口径） | 074-sink-mode.md §7.0 P-12/P-13；078-graph-view-ruling.md §6 |
