        ...
        <div class="fixed inset-0 z-[60] flex items-start justify-center bg-black/40 p-4 pt-[20vh]" @click.self="CloseOverlay">
          <div class="w-full max-w-xl overflow-hidden rounded-lg border bg-card shadow-lg">
            <div class="flex items-center gap-2 border-b px-3 py-2">
              ...
              <input class="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" v-model="query" :type="'text'" :placeholder="'Type a command or recent file...'" @keydown.down.prevent="NextItem" @keydown.enter.prevent="ExecuteSelected" @input="QueryInput($event)" @keydown.up.prevent="PrevItem" />
            </div>
              ...
              <component :is="(ul_tag) as any" class="max-h-[50vh] overflow-y-auto py-1">
                <component :is="(li_tag) as any" :class="(item.idx == selected_index ? 'cursor-pointer px-3 py-2 text-sm bg-accent text-accent-foreground' : 'cursor-pointer px-3 py-2 text-sm text-foreground hover:bg-accent/50')" @mouseenter="HoverItem(item)" @click="Execute(item)" v-for="item in filtered">
                  <div class="flex items-center gap-2">
                    <PaletteIcon :icon="item.icon" :class="'h-4 w-4 shrink-0 opacity-70'" :key="'PaletteIcon-1-' + (((item as any)?.id ?? item))" />
