    ...
    <div class="rounded-lg border bg-background/50 p-2.5">
      ...
      <template v-if="show_list">
        <component :is="(ul_tag) as any" class="space-y-0.5">
          <component :is="(li_tag) as any" class="cursor-pointer truncate rounded px-1.5 py-0.5 text-xs text-foreground/80 transition-colors hover:bg-accent hover:text-foreground" :key="h.content" :style="({ paddingLeft: h.pad } as any)" :title="h.content" @click="ScrollTo(h.content)" v-for="h in headings">
            <span>{{ h.content }}</span>
          </component>
        </component>
      </template>
